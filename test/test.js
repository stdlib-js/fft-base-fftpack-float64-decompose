...
tape( 'the function correctly handles stride and offset parameters', function test( t ) {
	...
	factors = new Float64Array( 11 );

	so = 2;
	oo = 1;
	nf = decompose( 12, 4, INITIAL, 1, 0, factors, so, oo );

	expected = new Float64Array( [ 0.0, 12.0, 0.0, 2.0, 0.0, 3.0, 0.0, 4.0, 0.0, 0.0, 0.0 ] ); // eslint-disable-line max-len

	t.strictEqual( nf, 2, 'returns expected value' );
	t.strictEqual( isSameFloat64Array( factors, expected ), true, 'returns expected value' );
